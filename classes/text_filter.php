...
use filter_translations\course_translation_policy;
...
class text_filter extends \moodle_text_filter {
     ...
     * @throws dml_exception
     */
    public static function skiptranslations(?\context $context = null) {
        global $PAGE, $SCRIPT;
        ...
        if (isset($skip[$contextid])) {
            return $skip[$contextid];
        }

        $skip[$contextid] = false;

        if (!course_translation_policy::for_context($context)->translation_enabled()) {
            $skip[$contextid] = true;
        }
        ...
        if (!$skip[$contextid] && in_array($SCRIPT,
            preg_split(
                "/\r?\n/",
     ...
     * @throws dml_exception
     */
    public static function skiplanguage(?\context $context = null) {
        global $PAGE;

        static $skipthislang = [];

        $context = $context ?? $PAGE->context;
        $cachekey = ($context->id ?? 0) . '_' . current_language();
        if (isset($skipthislang[$cachekey])) {
            return $skipthislang[$cachekey];
        }

        $skipthislang[$cachekey] = false;

        if (!course_translation_policy::for_context($context)->language_enabled(current_language())) {
            $skipthislang[$cachekey] = true;
        }
    ...
    public function filter($text, array $options = []) {
        ...
        if (self::skiptranslations($this->context) || self::skiplanguage($this->context) ||
                strpos($text, self::ENCODEDSEPERATOR . self::ENCODEDSEPERATOR) !== false) {
            return $text;
        }
    ...
    /**
     * Given a hash, create the translation span block.
     *
     * @param $hash The translation hash to use
     * @return string The translation span HTML string
     */
    public function createtranslationspan($hash) {
        return '<p class="translationhash"><span data-translationhash="' . $hash . '"></span></p>';
    }
